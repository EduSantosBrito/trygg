...
const NetworkErrorDisplay = Component.gen(function* (
...
const ValidationErrorDisplay = Component.gen(function* (
  Props: ComponentProps<{
...
const UnknownErrorDisplay = Component.gen(function* (
  Props: ComponentProps<{
    error: UnknownError;
  }>,
) {
...
const RiskyComponent = Effect.fn("RiskyComponent")(function* (
  ...
  if (shouldFail === "network") {
    return yield* Effect.fail(new NetworkError({ url: "/api/data", status: 500 }));
  }

  if (shouldFail === "validation") {
    return yield* Effect.fail(new ValidationError({ field: "email", message: "Invalid format" }));
  }
...
const TriggerButton = Component.gen(function* (
  Props: ComponentProps<{
    label: string;
  ...
  return (
    <button
      ...
      style={variant === "danger" ? { background: theme.errorText, color: "white" } : {}}
      onClick={onClick}
    >
      {label}
    </button>
