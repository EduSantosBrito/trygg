...
const SignupForm = Component.gen(function* () {
  ...
  const onEmailInput = (event: Event) =>
    event.target instanceof HTMLInputElement
      ? Signal.set(email, event.target.value)
      : Effect.void
  ...
  const onSubmit = (event: Event) =>
    Effect.gen(function* () {
      ...
      Effect.catch((cause) => Signal.set(error, String(cause))),
    )
...
const validateEmail = (raw: string): Effect.Effect<string, EmailRequired | EmailInvalid> =>
  raw.trim() === "" ? Effect.fail(new EmailRequired()) :
  !raw.includes("@") ? Effect.fail(new EmailInvalid({ email: raw })) :
  Effect.succeed(raw)

const validatePassword = (raw: string): Effect.Effect<string, PasswordTooShort> =>
  raw.length < 8 ? Effect.fail(new PasswordTooShort({ min: 8 })) :
  Effect.succeed(raw)
...
class HttpClient extends Context.Tag("HttpClient")<HttpClient, { readonly request: (path: string) => Effect.Effect<string> }>() {}
