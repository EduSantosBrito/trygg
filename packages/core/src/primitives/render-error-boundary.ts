...
interface RenderErrorBoundaryDeps {
...
export const renderErrorBoundary = (
  ...
  onError: ((cause: Cause.Cause<unknown>) => Effect.Effect<void, unknown, unknown>) | null,
  parent: Node,
  renderContext: RenderContext,
  context: Context.Context<unknown> | null,
  deps: RenderErrorBoundaryDeps,
): Effect.Effect<RenderResult, unknown, unknown> =>
  Effect.gen(function* () {
    ...
    ): Effect.Effect<{ result: RenderResult; scope: Scope.Closeable } | null, unknown, unknown> =>
      Effect.gen(function* () {
    ...
    const mountFallbackForCause = Effect.fnUntraced(function* (cause: Cause.Cause<unknown>) {
      yield* Debug.log({ event: "render.errorboundary.caught", reason: String(cause) });
