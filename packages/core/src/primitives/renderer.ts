 ...
 * Narrows to Effect<unknown> (defaults E=never, R=never) since prop Effects
 * are expected to be fully provided with no error channel.
 * @internal
 */
    ...
    Match.tag("Intrinsic", ({ tag, props, children }) =>
      Effect.gen(function* () {
        ...
        if (shouldHoist) {
          ...
          // as RenderResult.node or parent/Fragment will move it back into the body.
          const anchor = document.createComment(`head:${tag}`);
          parent.appendChild(anchor);

          // Cleanup: children, props, and anchor — node removal handled by Head scope finalizer
          return {
            node: anchor,
            cleanup: Effect.gen(function* () {
              for (const child of childResults) {
        ...
        // Normal element — append to parent
        parent.appendChild(node);
