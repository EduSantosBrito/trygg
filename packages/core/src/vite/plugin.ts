...
const EFFECT_UI_API_ID = "effect-ui/api";
...
export class ApiClient extends Context.Tag("effect-ui/ApiClient")<ApiClient, never>() {}
...
// Function-call inference correctly resolves the HttpApiClient type.
// (conditional type extraction in declare module blocks does not work)
...
export class ApiClient extends Context.Tag("effect-ui/ApiClient")<ApiClient, ApiClientService>() {}
...
export class ApiClient extends Context.Tag("effect-ui/ApiClient")() {}
...
export const generateEntryModule = (
  ...
  Effect.gen(function* () {
    ...
    // Compute relative import path from .effect-ui/ to the routes file
    const routesImport =
...
import { mountDocument, Component } from "effect-ui"
 ...
 */
/**
 * Plugin options for effect-ui.
 * @since 1.0.0
 */
export interface EffectUIOptions {
  /**
   * Path to the routes file (relative to project root).
   * When set, enables route type generation and code splitting transforms.
   *
   * @example
   * ```ts
...
export const effectUI = (options?: EffectUIOptions): Plugin => {
  ...
  return {
    ...
    async configResolved(resolvedConfig) {
      ...
      routesFilePath =
        options?.routes !== undefined ? nodePath.resolve(config.root, options.routes) : undefined;
